...
/datum/action/innate/ability/humanoid_customization/proc/change_form()
	...
	if(select_alteration == "Body Color")
		...
		if(iscultist(H) && HAS_TRAIT(H, TRAIT_CULT_EYES))
			to_chat(H, span_cultlarge("\"I do not need you to hide yourself anymore, relish my gift.\""))
			return
		...
		var/operation = tgui_input_list(owner, "Select organ operation.", "Organ Manipulation", list("add sexual organ", "remove sexual organ"))
		switch(operation)
			if("add sexual organ")
		...
		var/new_length = tgui_input_number(owner, "Penis length in centimeters:\n([min_D]-[max_D])", "Genital Alteration", H.dna.features["cock_length"], max_D, min_D)
		if(new_length)
		...
		var/new_size = tgui_input_number(owner, "Testicles size:\n([min_B]-[max_B])", "Genital Alteration", H.dna.features["balls_size"], max_B, min_B)
		if(new_size)
...
/// SPLURT EDIT START
		...
		var/new_length = tgui_input_number(owner, "Butt size:\n([min_B]-[max_B])", "Genital Alteration", H.dna.features["butt_size"], max_B, min_B)
		if(new_length)
		...
		var/new_length = tgui_input_number(owner, "Belly size:\n([min_belly]-[max_belly])", "Genital Alteration", H.dna.features["belly_size"], max_belly, min_belly)
		if(new_length)
	...
	//BLUEMOON CHANGE изменение размера требует время, сколько слишком сильно отличается в особенностях механик от сплюрта, как и в тематике сервера, чтобы давать возможность его так легко изменять
		...
		if (size_normalized)
			to_chat(owner, span_warning("The normalizer prevents you from adjusting your entire body's size."))
			return
		else
	...
	//BLUEMOON CHANGE END
		...
		var/new_horns = tgui_input_list(owner, "Choose your character's horns:", "Character Preference", GLOB.horns_list)
		if(new_horns)
		...
		if(CONFIG_GET(flag/allow_custom_skintones))
			choices += "custom"
		var/new_s_tone = tgui_input_list(owner, "Choose your character's skin tone: (This is different than the Body Color option, which changes your character's mutant colors)", "Character Preference", choices)
		if(new_s_tone)
			if(new_s_tone == "custom")
				...
				if(custom_tone)
					...
					if(ReadHSV(temp_hsv)[3] < ReadHSV("#333333")[3] && CONFIG_GET(flag/character_color_limits)) // rgb(50,50,50) //SPLURT EDIT
						to_chat(owner,span_danger("Invalid color. Your color is not bright enough."))
					else
		...
		var/leg_choice = tgui_alert(owner, "Choose your character's leg type:", "Leg Type Alteration", list("Plantigrade", "Digitigrade"))
		if(leg_choice)
			switch(leg_choice)
				if("Plantigrade")
