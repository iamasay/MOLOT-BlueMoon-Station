...
/obj/item/necromantic_stone/attack(mob/living/carbon/human/M, mob/living/carbon/human/user)
	...
	to_chat(M, "<span class='userdanger'>[user.ru_who(TRUE)] your master now, assist [user.ru_na()] even if it costs you your new life!</span>")
...
/obj/item/voodoo/suicide_act(mob/living/carbon/user)
	user.visible_message("<span class='suicide'>[user] links the voodoo doll to себя and sits on it, infinitely crushing себя! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	user.gib()
	return(BRUTELOSS)
