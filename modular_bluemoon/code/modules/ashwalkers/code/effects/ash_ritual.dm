...
/datum/ash_ritual/proc/ritual_start(obj/effect/ash_rune/rune)
	...
	rune.balloon_alert_to_viewers("ритуал начался...")
	...
	for(var/checked_component in required_components)
		...
		if(is_type_in_list(atom_check, consumed_components))
			if(istype(atom_check, /obj/item/stack))
				...
				qdel(atom_check)
			checked_rune.balloon_alert_to_viewers("[checked_component] компонент был потреблен...")

		else
			checked_rune.balloon_alert_to_viewers("[checked_component] компонент был проверен...")
...
/datum/ash_ritual/proc/ritual_fail(obj/effect/ash_rune/failed_rune)
	...
	failed_rune.balloon_alert_to_viewers("ритуал не удался...")
...
/datum/ash_ritual/proc/ritual_success(obj/effect/ash_rune/success_rune)
	...
	success_rune.balloon_alert_to_viewers("ритуал прошел успешно...")
	log_game("[name] ритуал был успешно активирован.")
