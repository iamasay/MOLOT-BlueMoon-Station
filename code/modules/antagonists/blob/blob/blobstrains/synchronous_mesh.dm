...
/datum/reagent/blob/synchronous_mesh/reaction_mob(mob/living/M, method=TOUCH, reac_volume, show_message, touch_protection, mob/camera/blob/O, affected_bodypart)
	reac_volume = ..()
	M.apply_damage(0.2*reac_volume, BRUTE, wound_bonus=5)
	if(M && reac_volume)
		for(var/obj/structure/blob/B in range(1, M)) //if the target is completely surrounded, this is 2.4*reac_volume bonus damage, total of 2.6*reac_volume
			if(M)
				...
				M.apply_damage(0.3*reac_volume, BRUTE, wound_bonus=5)
