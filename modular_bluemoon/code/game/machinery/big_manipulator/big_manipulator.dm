/obj/machinery/big_manipulator
	...
	icon = 'modular_bluemoon/icons/obj/machines/big_manipulator_parts/big_manipulator_core.dmi'
	icon_state = "core"
	density = TRUE
...
/obj/machinery/big_manipulator/proc/create_new_task(mob/user, task_type, turf/new_turf)
	...
	var/obj/item/stock_parts/manipulator/locate_manipulator = locate() in component_parts
	var/manipulator_tier = locate_manipulator ? locate_manipulator.rating : 1

	var/datum/manipulator_task/new_task
	var/needs_turf = task_type in list(TASK_TYPE_PICKUP, TASK_TYPE_DROP, TASK_TYPE_THROW)
...
/obj/machinery/big_manipulator/update_overlays()
	. = ..()
	if(panel_open)
		. += "panel"
...
/obj/machinery/big_manipulator/proc/process_upgrades()
	var/obj/item/stock_parts/manipulator/locate_manipulator = locate() in component_parts
	if(!locate_manipulator)
		return

	var/manipulator_tier = locate_manipulator.rating
	...
	active_power_usage = 100 * BASE_POWER_USAGE * manipulator_tier
...
/obj/machinery/big_manipulator/screwdriver_act(mob/living/user, obj/item/tool)
	...
	update_icon()
	to_chat(user, panel_open ? span_notice("You unscrew the maintenance panel of [src].") : span_notice("You screw the maintenance panel of [src] back on."))
	return TRUE
...
/obj/machinery/big_manipulator/attackby(obj/item/some_item, mob/user, params)
	if(istype(some_item, /obj/item/disk/manipulator))
		if(on || stopping)
		...
		if(task_disk)
			task_disk.forceMove(drop_location())
			task_disk = null
		if(!user.transferItemToLoc(some_item, src))
			return
		task_disk = some_item
...
/obj/machinery/big_manipulator/proc/adjust_param_for_task(task_ref, param, value, mob/user)
	...
	switch(param)
		if("set_name")
			if(!value)
				return FALSE
			target_task.name = sanitize_name(value)
			return TRUE
		...
		if("cycle_overflow_status")
			...
			cycle_target_task.overflow_status = cycle_value(cycle_target_task.overflow_status, list(POINT_OVERFLOW_ALLOWED, POINT_OVERFLOW_HELD, POINT_OVERFLOW_FORBIDDEN))
			return TRUE
