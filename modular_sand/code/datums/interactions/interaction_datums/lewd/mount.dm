	...
	if(user.is_fucking(partner, THIGH_SMOTHERING) || user.is_fucking(partner, CUM_TARGET_MOUTH))
		...
		switch(fucktarget)
			...
			if("penis")
				if(user.has_penis() || user.has_strapon())
					message = pick(list(
						"похотливо облизывается и поправляет своё положение на личике <b>[partner]</b>, после чего пару раз сжимает свои бёдра, [user.has_penis(TRUE) ? "выделяя немного предэякулята на лицо <b>[partner]</b>." : "шлепая дилдо по лицу <b>[partner]</b>."]", // BLUEMOON EDIT откуда предэякулят у страпона Bruh
