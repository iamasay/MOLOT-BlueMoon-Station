...
/datum/ai_behavior/item_escape_grasp/perform(seconds_per_tick, datum/ai_controller/controller)
	...
	if(!istype(item_holder)) //We're no longer being held. abort abort!!
...
///Haunted item: move closer to the haunt target and hurl the pawn at them.
/datum/ai_behavior/haunted_item_attack
	required_distance = 3
...
/datum/ai_behavior/haunted_item_attack/setup(datum/ai_controller/controller, target_key, throw_count_key)
	. = ..()
...
/datum/ai_behavior/haunted_item_attack/perform(seconds_per_tick, datum/ai_controller/controller, target_key, throw_count_key)
	var/obj/item/item_pawn = controller.pawn
...
/datum/ai_behavior/haunted_item_attack/finish_action(datum/ai_controller/controller, succeeded, target_key, throw_count_key)
