/client/proc/show_spawn_text_overlay(duration = 5 SECONDS)
	...
	var/job_title = mob.job || "Неизвестно"
	if(mob.mind?.assigned_role)
		job_title = mob.mind.assigned_role
	var/station = station_name()
	var/area_name = get_area_name(mob, format_text = TRUE) || "Неизвестно"
	var/time_date = "[GLOB.current_date_string] [STATION_TIME_TIMESTAMP("hh:mm:ss", world.time)]"
	var/text = {"
		[mob_name] - [job_title]
		[station]
		[area_name]
