...
/datum/element/food_trash/proc/async_generate_trash(atom/source)
	var/obj/item/trash_item = generate_trash_procpath ? call(source, generate_trash_procpath)() : new trash(get_turf(source))

	if(isliving(source.loc))
		var/mob/living/food_holding_mob = source.loc
		food_holding_mob.dropItemToGround(source)
