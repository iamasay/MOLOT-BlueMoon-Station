...
/mob/living/carbon/true_devil/resist_buckle()
	if(buckled)
		...
		visible_message("<span class='warning'>[src] easily breaks out of [ru_ego()] handcuffs!</span>", \
					"<span class='notice'>With just a thought your handcuffs fall off.</span>")

/mob/living/carbon/true_devil/canUseTopic(atom/movable/M, be_close=FALSE, no_dextery=FALSE, no_tk=FALSE, check_resting=FALSE, silent = FALSE)
	if(incapacitated())
		if(!silent)
...
/mob/living/carbon/true_devil/flash_act(intensity = 1, override_blindness_check = 0, affect_silicon = 0, visual = 0, type = /atom/movable/screen/fullscreen/tiled/flash, override_protection = 0, duration = 25)
...
/mob/living/carbon/true_devil/attack_ghost(mob/dead/observer/user as mob)
	if(ascended || (user.mind && user.mind.soulOwner == src.mind))
