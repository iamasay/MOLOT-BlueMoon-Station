...
/obj/machinery/implantchair/brainwash/implant_action(mob/living/C,mob/user)
	...
	user.log_message("has brainwashed [key_name(C)] with the objective '[objective]' using \the [src]", LOG_ATTACK, target = C)
	C.log_message("has been brainwashed with the objective '[objective]' by [key_name(user)] using \the [src]", LOG_VICTIM, log_globally = FALSE, target = user)
