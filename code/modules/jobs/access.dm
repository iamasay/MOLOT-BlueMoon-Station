...
/proc/get_centcom_access(job)
	switch(job)
		...
		if("Special Ops Officer")
			return get_all_centcom_access()+list(ACCESS_CENT_BAR)
		if("Admiral")
			return get_all_centcom_access()+list(ACCESS_CENT_BAR)
...
/proc/get_all_accesses()
	return list(ACCESS_SECURITY, ACCESS_SEC_DOORS, ACCESS_BRIG, ACCESS_ARMORY, ACCESS_FORENSICS_LOCKERS, ACCESS_COURT, ACCESS_ENTER_GENPOP, ACCESS_LEAVE_GENPOP,
				...
				ACCESS_PRODUCTION_MEDICAL, ACCESS_PRODUCTION_SCIENCE, ACCESS_PRODUCTION_SECURITY, ACCESS_PRODUCTION_SERVICE, ACCESS_PRODUCTION_CARGO, ACCESS_PRODUCTION_ENGINEERING)
...
/proc/get_region_accesses(code)
	switch(code)
		...
		if(1) //station general
			return list(ACCESS_KITCHEN, ACCESS_BAR, ACCESS_HYDROPONICS, ACCESS_JANITOR, ACCESS_CHAPEL_OFFICE, ACCESS_CREMATORIUM, ACCESS_LIBRARY, ACCESS_THEATRE, ACCESS_LAWYER, ACCESS_PRODUCTION_SERVICE)
		if(2) //security
			return list(ACCESS_SEC_DOORS, ACCESS_WEAPONS, ACCESS_SECURITY, ACCESS_BRIG, ACCESS_ARMORY, ACCESS_FORENSICS_LOCKERS, ACCESS_COURT, ACCESS_HOS, ACCESS_ENTER_GENPOP, ACCESS_LEAVE_GENPOP, ACCESS_BRIGDOC, ACCESS_PRODUCTION_SECURITY)
		if(3) //medbay
			return list(ACCESS_MEDICAL, ACCESS_GENETICS, ACCESS_CLONING, ACCESS_MORGUE, ACCESS_CHEMISTRY, ACCESS_VIROLOGY, ACCESS_SURGERY, ACCESS_CMO, ACCESS_PSYCH, ACCESS_PRODUCTION_MEDICAL)
		if(4) //research
			return list(ACCESS_RESEARCH, ACCESS_TOX, ACCESS_TOX_STORAGE, ACCESS_GENETICS, ACCESS_ROBOTICS, ACCESS_XENOBIOLOGY, ACCESS_MINISAT, ACCESS_RD, ACCESS_NETWORK, ACCESS_PRODUCTION_SCIENCE)
		if(5) //engineering and maintenance
			return list(ACCESS_CONSTRUCTION, ACCESS_MAINT_TUNNELS, ACCESS_ENGINE, ACCESS_ENGINE_EQUIP, ACCESS_EXTERNAL_AIRLOCKS, ACCESS_TECH_STORAGE, ACCESS_ATMOSPHERICS, ACCESS_TCOMSAT, ACCESS_MINISAT, ACCESS_CE, ACCESS_PRODUCTION_ENGINEERING)
		if(6) //supply
			return list(ACCESS_MAILSORTING, ACCESS_MINING, ACCESS_MINING_STATION, ACCESS_MINERAL_STOREROOM, ACCESS_CARGO, ACCESS_QM, ACCESS_VAULT, ACCESS_PRODUCTION_CARGO)
		if(7) //command
...
/proc/get_access_desc(A)
	switch(A)
		...
		if(ACCESS_PRODUCTION_MEDICAL)
			return "Medical Production"
		if(ACCESS_PRODUCTION_SCIENCE)
			return "Science Production"
		if(ACCESS_PRODUCTION_SECURITY)
			return "Security Production"
		if(ACCESS_PRODUCTION_SERVICE)
			return "Service Production"
		if(ACCESS_PRODUCTION_CARGO)
			return "Cargo Production"
		if(ACCESS_PRODUCTION_ENGINEERING)
			return "Engineering Production"
