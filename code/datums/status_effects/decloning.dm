...
/datum/status_effect/decloning/tick()
	if(iscarbon(owner) && owner.reagents && owner.reagents.has_reagent(/datum/reagent/medicine/mutadone, MUTADONE_HEAL * 3))
		var/strike_restore = MUTADONE_HEAL * 3
		...
		owner.reagents.remove_reagent(/datum/reagent/medicine/mutadone, MUTADONE_HEAL * 3)
	...
	if(strikes_left >= 68)
		examine_text = "<span class='warning'>[owner.p_their()] body looks a bit deformed.</span>"
	else if(strikes_left >= 34)
		examine_text = "<span class='warning'>[owner.p_their()] body looks <b>very</b> deformed.</span>"
	else
		examine_text = "<span class='boldwarning'>[owner.p_their()] body looks severely deformed!</span>"
