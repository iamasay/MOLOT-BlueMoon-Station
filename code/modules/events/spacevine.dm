...
/datum/spacevine_mutation/borshevik
	name = "borshevik"
	hue = "#00ff11"
	severity = 10
	quality = NEGATIVE

/datum/spacevine_mutation/borshevik/on_cross(obj/structure/spacevine/holder, mob/living/crosser)
	if(issilicon(crosser))
		return
	if(prob(severity) && istype(crosser) && !isvineimmune(crosser))
		to_chat(crosser, span_alert("You accidentally touch the vine and feel a strange sensation."))
		crosser.reagents.add_reagent_list(list(/datum/reagent/drug/labebium = 10))

/datum/spacevine_mutation/borshevik/on_eat(obj/structure/spacevine/holder, mob/living/eater)
