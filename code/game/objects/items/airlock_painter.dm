...
/datum/asset/spritesheet_batched/decals
	name = "floor_decals"
	// Часть декалей односторонняя ("bot", "tile_fourcorners", "trimline_box_fill"), а
	// превью просит все четыре дирса. DM в таком случае тихо отдавал южный кадр, rust-g
	// же ругается на невалидный дир - жалобу глотаем, иначе сборка листа падает.
	// Фронтенд для таких декалей запрашивает только dir=SOUTH (nondirectional_decals),
	// так что в UI это ничего не меняет.
...
/datum/asset/spritesheet_batched/decals/tiles/insert_state(decal, dir, color)
	// Account for 8-sided decals.
	// turn() здесь про ВЫБОР дирса в DMI, а не про поворот картинки: у "__8"-стейтов
	// диагональные кадры уже нарисованы, крутить растр (rotate) было бы неверно.
