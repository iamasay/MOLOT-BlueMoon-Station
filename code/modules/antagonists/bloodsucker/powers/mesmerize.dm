...
/datum/action/cooldown/bloodsucker/targeted/mesmerize/proc/apply_effects(aggressor, victim, power_time)
	...
	to_chat(L, "<span class='mesmerize'>[target] застыл[target.ru_who() == "он"? "" : "а"] на месте от вашего гипнотического взгляда.</span>")
	...
	if(istype(target) && success)
		if(istype(L) && target.stat == CONSCIOUS && (target in L.fov_view(10))) // They Woke Up! (Notice if within view)
			to_chat(L, "<span class='mesmerize'>[target] выш[target.ru_who() == "он"? "ел" : "ла"] из транса.</span>")
