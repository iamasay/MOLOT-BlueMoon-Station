/datum/disease/chronic_illness
	...
	var/being_stealthy = TRUE

/datum/disease/chronic_illness/stage_act()
	. = ..()
	if(!.)
		return
	...
	switch(stage)
		...
		if(3)
			...
			if(prob(1))
				...
				SEND_SOUND(affected_mob, sound('sound/effects/genetics.ogg'))
				being_stealthy = FALSE
			if(prob(1))
