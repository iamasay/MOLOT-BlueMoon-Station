...
/datum/smite/bookify/effect(client/user, mob/living/carbon/human/target as mob)
	...
	var/mutable_appearance/transform_scanline = mutable_appearance('modular_splurt/icons/effects/effects.dmi', "transform_effect", color = "#FF3300")
	target.transformation_animation(book_appearance, time = BOOKIFY_TIME, transform_overlay=transform_scanline, reset_after=TRUE)
	addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(bookify), target), BOOKIFY_TIME)
