...
/obj/item/abductor/baton/proc/SleepAttack(mob/living/L,mob/living/user)
	if(L.incapacitated(TRUE, TRUE))
		if(L.anti_magic_check(FALSE, FALSE, TRUE, 0))
			...
			L.visible_message(span_danger("[user] tried to induced sleep in [L] with [src], but [L.ru_ego()] tinfoil protected [L.ru_na()]!"), \
		...
		if(L.anti_magic_check(FALSE, FALSE, TRUE, 0))
			...
			L.visible_message(span_danger("[user] tried to induce sleep in [L] with [src], but [L.ru_ego()] tinfoil completely protected [L.ru_na()]!"), \
