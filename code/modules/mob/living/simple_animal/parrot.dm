...
/mob/living/simple_animal/parrot/Polly/say(message, bubble_type,var/list/spans = list(), sanitize = TRUE, datum/language/language = null, ignore_spam = FALSE, forced = null)
	. = ..()
	if(. && !client && prob(5) && prob(5) && CONFIG_GET(string/chat_squawk_tag)) //Only the one true bird may speak across dimensions.
		send2chat(new /datum/tgs_message_content("Поли опять что-то кричит... \"[message]\""), CONFIG_GET(string/chat_squawk_tag))
