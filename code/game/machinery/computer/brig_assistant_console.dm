...
/obj/machinery/computer/brig_assistant_console/ui_act(action, params)
	...
	switch(action)
		...
		if("pay_fine")
			...
			// логи
			GLOB.data_core.append_sec_logs(sec.fields["id"], "%%GEN_AUTH%% оплатил [amount] кр. по штрафу \"[fine.crimeName]\" (остаток [fine.fine] кр.) через консоль брига", holder, "Гражданин")
			if(fine.fine == 0)
				GLOB.data_core.append_sec_logs(sec.fields["id"], "Штраф \"[fine.crimeName]\" полностью погашен гражданином [holder]", "Система", "Автоматика")
				say("Штраф \"[fine.crimeName]\" полностью оплачен. Спасибо.")
				playsound(src, 'sound/machines/terminal_success.ogg', 50, FALSE)
				print_fine_receipt(sec, fine, amount, holder)
				if(security_radio)
					security_radio.talk_into(src, "Гражданин [holder] полностью оплатил штраф \"[fine.crimeName]\" ([fine.paid + fine.fine] кр.).")
				...
				playsound(src, 'sound/machines/terminal_success.ogg', 50, FALSE)
				if(security_radio)
					security_radio.talk_into(src, "Гражданин [holder] оплатил [amount] кр. по штрафу \"[fine.crimeName]\" (остаток [fine.fine] кр.).")
				print_fine_receipt(sec, fine, amount, holder)
		...
		if("pay_fine_full")
			...
			if(sec_account)
				sec_account.adjust_money(amount, "Поступление штрафа от [holder]")
			GLOB.data_core.payFine(sec.fields["id"], cdataid, amount)
			GLOB.data_core.append_sec_logs(sec.fields["id"], "%%GEN_AUTH%% полностью оплатил штраф \"[fine.crimeName]\" ([amount] кр.) через консоль брига", holder, "Гражданин")
			say("Штраф полностью оплачен.")
			playsound(src, 'sound/machines/terminal_success.ogg', 50, FALSE)
			print_fine_receipt(sec, fine, amount, holder)
			if(security_radio)
				security_radio.talk_into(src, "Гражданин [holder] полностью оплатил штраф \"[fine.crimeName]\".")
