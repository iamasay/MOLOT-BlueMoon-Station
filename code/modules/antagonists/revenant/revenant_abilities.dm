	...
	if(do_after(src, rand(10, 20), target)) //did they get deleted in that second?
		...
		if(do_after(src, rand(15, 20), target)) //did they get deleted NOW?
			...
			if(do_after(src, rand(15, 25), target)) //how about now
				if(!target.stat)
					to_chat(src, "<span class='revenwarning'>[target.ru_who(TRUE)] now powerful enough to fight off your draining.</span>")
...
/obj/effect/proc_holder/spell/aoe_turf/revenant/defile/proc/defile(turf/T)
	...
	if(!isplatingturf(T) && !istype(T, /turf/open/floor/engine/cult) && isfloorturf(T) && prob(15))
		...
		if((floor.turf_flags & TURF_INTACT) && floor.floor_tile)
