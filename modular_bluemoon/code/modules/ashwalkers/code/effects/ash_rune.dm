...
/obj/effect/ash_rune
	name = "ash rune"
	desc = "Остатки цивилизации, которая когда-то была достаточно могущественной, чтобы использовать странную энергию для трансмутаций."
	icon = 'modular_bluemoon/code/modules/ashwalkers/icons/ash_ritual.dmi'
	icon_state = "rune"
	anchored = TRUE
...
/obj/effect/ash_rune/examine(mob/user)
	. = ..()
	if(!current_ritual)
		. += span_notice("<br>В данный момент ритуал не выбран — используйте центральную руну, чтобы выбрать ритуал.")
		return
	. += span_notice("<br>Текущий ритуал: [current_ritual.name]")
	. += span_notice(current_ritual.desc)
	. += span_warning("<br>Необходимые компоненты следующие:")
...
/obj/effect/ash_rune/attack_hand(mob/living/user, list/modifiers)
	...
	if(current_ritual && is_species(user, /datum/species/lizard/ashwalker))
		...
		return
	current_ritual = tgui_input_list(user, "Выберите ритуал, чтобы начать...", "Выбор ритуала", GLOB.ash_rituals)
	if(!current_ritual)
		return
	current_ritual = GLOB.ash_rituals[current_ritual]
	balloon_alert_to_viewers("ритуал выбран — для получения дополнительной информации изучите центральную руну.")

// this is solely for aesthetics... though the central rune will check the directions, of which this is on
/obj/effect/side_rune
	desc = "Эта руна, кажется, обладает какой-то странной пустотой."
	icon = 'modular_bluemoon/code/modules/ashwalkers/icons/ash_ritual.dmi'
	resistance_flags = INDESTRUCTIBLE | LAVA_PROOF | FIRE_PROOF | UNACIDABLE | ACID_PROOF | FREEZE_PROOF
	anchored = TRUE
